...
with tab1:
    ...
    st.write(f"<h3 style='color:#336699;'>Sopimusten kokonaisarvo yhteensä:{total_sopimus:.2f} €</h3>", unsafe_allow_html=True)
...
with tab2:
    ...
    st.write(f"<h3>Myyntiennusteen kokonaisarvo yhteensä: {total_ennuste:.2f} €</h3>", unsafe_allow_html=True)
    st.write(f"<h3 style='color:#336699;'> Kokonaisarvo (sopimukset + myyntiennuste): {total_sopimus + total_ennuste:.2f} €</h3>", unsafe_allow_html=True)
with tab3:
    st.write("Arvioi liiketoimintasi kulut kuukaudessa (ilman ALV:tä) ja veroprosentti.")

    # Alustetaan lista, jos sitä ei ole vielä
    if "asiakkaat_palkkaennuste" not in st.session_state:
        st.session_state.asiakkaat_palkkaennuste = []

    # Lomake kulun lisäämiseen
    with st.form("liiketoiminnan_kulut_palkka"):
        kulu = st.text_input("Kulu", value=st.session_state.get("kulu_ennuste", ""), key="kulu_ennuste")
        a_hinta = st.number_input("Kulun á hinta (ilman alv., €)", min_value=0.0, step=1.0, format="%.2f", value=st.session_state.get("a_hinta", 0.0), key="a_hinta")
    ...
    # Näytetään lisätyt kulut
    st.subheader("Lisätyt kulut:")
    kelvolliset_kulut = [
        k for k in st.session_state.asiakkaat_palkkaennuste
        if isinstance(k, dict) and all(avain in k for avain in ['kulu', 'a_hinta', 'maara', 'kokonaisarvo'])
    ]

    if kelvolliset_kulut:
        for k in kelvolliset_kulut:
            st.write(f"- {k['kulu']}: {k['a_hinta']:.2f} € × {k['maara']} kpl = {k['kokonaisarvo']:.2f} €")

        # Poistettava kulu
        kululista_str = [f"{k['kulu']} ({k['a_hinta']:.2f} € × {k['maara']} kpl)" for k in kelvolliset_kulut]
        poistettava_kulu = st.selectbox("Valitse poistettava kulu", kululista_str + ["-"], index=len(kululista_str))

        if poistettava_kulu != "-":
            if st.button("Poista valittu kulu"):
                st.session_state.asiakkaat_palkkaennuste = [
                    k for k in kelvolliset_kulut
    ...
    bruttopalkka = kokonaismyynti - (kulujen_summa / 12)
    ...
    st.markdown(f"<h2 style='color:#336699;'>Arvioitu nettopalkka kuukaudessa: {nettopalkka:.2f} €</h2>", unsafe_allow_html=True)
